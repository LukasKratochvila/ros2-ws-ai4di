...
class KalmanFilter3D(object):
    """
    A simple Kalman filter for tracking 3d bounding boxes in space.

    The 12-dimensional state space

        x, y, z, w, h, l, vx, vy, vz, vw, vh, vl

    contains the bounding box center position (x, y, z), width w, height h, lenght l
    and their respective velocities.

    Object motion follows a constant velocity model. The bounding box location
    (x, y, z, w, h, l) is taken as direct observation of the state space (linear
    observation model).
    ...
    def initiate(self, measurement):
        ...
        ----------
        measurement : ndarray
            Bounding box coordinates (x, y, z, w, h, l) with center position (x, y),
            width w, height h, and lenght l.
    ...
    def gating_distance(self, mean, covariance, measurements,
        ...
        measurements : ndarray
            ...
            format (x, y, z, w, h, l) where (x, y) is the bounding box center
            position, w the width, h the height, and l the lenght.
        only_position : Optional[bool]
            If True, distance computation is done with respect to the bounding
            box center position only.
