...
private:
  void Get_AABB_(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ptr, pcl::Indices *indices, pcl::PointXYZ *center_point, pcl::PointXYZ *min_point, pcl::PointXYZ *max_point) const
    ...
    {
      center_point->x += (*cloud_ptr)[(*indices)[i_point]].x;
      center_point->y += (*cloud_ptr)[(*indices)[i_point]].y;
      center_point->z += (*cloud_ptr)[(*indices)[i_point]].z;
    ...
    center_point->x /= number_of_points;
    center_point->y /= number_of_points;
    center_point->z /= number_of_points;
  }
  void topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) const
  {
    ...
    if (!OBB)
    {
      std::vector<pcl::PointXYZ> center_list, size_list;
      for (auto &it : cluster_indices)
      {
        pcl::PointXYZ centerPoint = {NAN, NAN, NAN};
        pcl::PointXYZ minPoint = {NAN, NAN, NAN};
        pcl::PointXYZ maxPoint = {NAN, NAN, NAN};

        Get_AABB_(cloud, &it.indices, &centerPoint, &minPoint, &maxPoint);
        center_list.emplace_back(centerPoint);
        maxPoint.x -= minPoint.x;
        maxPoint.y -= minPoint.y;
        maxPoint.z -= minPoint.z;
        size_list.emplace_back(maxPoint);
      }
      for (size_t i = 0; i < center_list.size(); i++)
      {
        cx.emplace_back(center_list.at(i).x);
        cy.emplace_back(center_list.at(i).y);
        cz.emplace_back(center_list.at(i).z);
        dx.emplace_back(size_list.at(i).x);
        dy.emplace_back(size_list.at(i).y);
        dz.emplace_back(size_list.at(i).z);
        ox.emplace_back(0.0);
        oy.emplace_back(0.0);
        oz.emplace_back(0.0);
        ow.emplace_back(1.0);
    ...
    else
    {
      ...
      pcl::PointXYZ centerPoint, minPoint, maxPoint, center_point, min_point, max_point;
      ...
      for (pcl::PointIndices &it : cluster_indices)
      {
        ...
        ox.emplace_back(cos(angle/2));
        oy.emplace_back(sin(angle/2));
        oz.emplace_back(0);
        ow.emplace_back(0);
      }
    }
