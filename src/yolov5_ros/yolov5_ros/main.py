...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.declare_parameter("input_img_topic", "/yolov5/image_raw")
        self.declare_parameter("output_det_topic", "/yolov5/bounding_boxes")
        ...
        self.pub_bbox = self.create_publisher(Detection2DArray, self.output_det_topic, 10)
        self.sub_image = self.create_subscription(Image, self.input_img_topic, self.image_callback,10)
    ...
    def yolovFive2bboxes_msgs(self, bboxes:list, scores:list, cls:list, img_header:Header):
        ...
        for score, i in zip(scores,range(len(scores))):
            if cls[i] in self.processed_classes:
                #one_box = BoundingBox()
                ...
                one_box.results.append(hypothesis)
                one_box.bbox.center.x = (bboxes[0][i] + bboxes[2][i])/2
                one_box.bbox.center.y = (bboxes[1][i] + bboxes[3][i])/2
                one_box.bbox.size_x = (bboxes[2][i] - bboxes[0][i])
                one_box.bbox.size_y = (bboxes[3][i] - bboxes[1][i])
                bboxes_msg.detections.append(one_box)
        return bboxes_msg
